...
"""
 This class is a utility to select the optimum number of missions for ISRO with in specified budget
...
class ISROMissionApprover:
    ...
    def find_maximum_profit(self, budgets, values, allocated_budget, no_of_missions):
        ...
        if budgets[previous_mission_index] <= allocated_budget:
            ...
            include = values[previous_mission_index] + self.find_maximum_profit(budgets, values,
                                                                                allocated_budget - budgets[
            ...
            self.mission_value_matrix[no_of_missions][allocated_budget] = self.find_maximum_profit(budgets, values,
                                                                                                   allocated_budget,
                                                                                                   previous_mission_index)
            return self.mission_value_matrix[no_of_missions][allocated_budget]
    ...
    def prepare_output(self):
        ...
        self.total_value = self.mission_value_matrix[itemNo][self.allocated_budget] if \
            self.mission_value_matrix[itemNo][self.allocated_budget] >= 0 else 0
        current_profit = self.total_value
        self.selected_missions = []

        while itemNo > 0:
            if (itemNo == 1 or current_profit != self.mission_value_matrix[itemNo - 1][self.remaining_budget]) and \
                    self.budgets[itemNo - 1] < self.remaining_budget:  # item was included
